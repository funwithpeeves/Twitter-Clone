...
const Modal = ({ tweet, close }) => {
  ...
  return (
    <div className="fixed inset-0 w-full h-full bg-zinc-700 z-[9999] grid place-content-center bg-opacity-70">
      <div className="bg-black rounded-md py-10 px-8 w-3/4 max-w-[600px] min-h-[60vh] max-h-[80vh] flex flex-col">
        <div className="flez justify-between">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 mt-10 justify-between">
          <div className="flex flex-col">
            ...
            {!isPicDeleting && tweet.imageContent ? (
              <button onClick={() => setIsPicDeleting(true)} className="mt-10 bg-orange-500 rounded-md p-2 hover:bg-orange-600">
                Resmi Kaldır 
              </button>
            ) : (
              <>
          ...
          <div className="flex justify-end gap-5">
            <button onClick={close} className="bg-gray-500 py-2 px-4 rounded-full hover:bg-gray-600" type="button">
              Vazgeç
            </button>
            <button className="bg-blue-500 py-2 px-4 rounded-full hover:bg-blue-600 min-w-[80px] flex justify-center items-center" type="submit">
              Kaydet
            </button>
          </div>
        </form>
